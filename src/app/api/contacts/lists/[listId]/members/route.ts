...
import { ContactListMembersSchema } from "@/server/validators/contact-list.validator";
...
function listErrorResponse(error: unknown) {
  ...
  if (error instanceof z.ZodError) {
    return NextResponse.json(
      { ok: false, code: "VALIDATION_ERROR", message: "Invalid member payload", errors: error.flatten().fieldErrors },
      { status: 400 },
    );
  }
...
export async function POST(request: Request, context: RouteContext) {
  try {
    ...
    const body = ContactListMembersSchema.parse(await request.json());
