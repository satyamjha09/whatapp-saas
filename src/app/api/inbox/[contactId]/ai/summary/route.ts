...
import {
  generateInboxConversationSummary,
  getLatestInboxConversationSummary,
} from "@/server/services/inbox-ai-summary.service";
...
export async function POST(request: Request, { params }: RouteContext) {
  try {
    ...
    const summary = await generateInboxConversationSummary({
      companyId: context.membership.companyId,
      contactId,
    ...
    return NextResponse.json({
      message:
        summary.status === "FAILED"
          ? "AI summary failed without blocking the inbox"
...
export async function GET(request: Request, { params }: RouteContext) {
  try {
    ...
    const { contactId } = await params;
    const summary = await getLatestInboxConversationSummary({
      companyId: context.membership.companyId,
      contactId,
    });
